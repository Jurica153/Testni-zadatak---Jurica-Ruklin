...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email)) {
        ...
        if ($stmt->fetchColumn() > 0) {
            $error = "E-mail adresa već postoji na mailing listi.";
        } else {
            $stmt = $pdo->prepare("INSERT INTO subscribers (email, name, is_active) VALUES (:email, :name, :is_active)");
            if ($stmt->execute(['email' => $email, 'name' => $name, 'is_active' => $is_active])) {
                redirect('index.php?message=Pretplatnik uspješno dodan!');
            } else {
                $error = "Greška prilikom dodavanja pretplatnika.";
            }
        }
    }
}
...
<div class="container">
    <h2>Dodaj novog pretplatnika</h2>
    <?php if (!empty($error)): ?>
    ...
    <form action="add.php" method="POST">
        ...
        <div class="form-group">
            <label for="name">Ime (opcionalno):</label>
            <input type="text" id="name" name="name" value="<?php echo isset($_POST['name']) ? htmlspecialchars($_POST['name']) : ''; ?>">
        </div>
        <div class="form-group">
            <label for="is_active">
                <input type="checkbox" id="is_active" name="is_active" value="1" <?php echo (isset($_POST['is_active']) || !isset($_POST['email'])) ? 'checked' : ''; ?>> Aktivan
            </label>
        </div>
        <button type="submit">Dodaj pretplatnika</button>
