...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($email) || empty($password)) {
        ...
        if ($stmt->fetchColumn() > 0) {
            $error = "Korisničko ime ili e-mail adresa već postoji.";
        } else {
            ...
            if ($stmt->execute(['username' => $username, 'email' => $email, 'password_hash' => $password_hash, 'role' => $role, 'is_active' => $is_active])) {
                redirect('/admin/users/index.php?message=Korisnik uspješno dodan!');
            } else {
...
<div class="container">
    ...
    <?php endif; ?>
    <form action="<?php echo BASE_URL; ?>/admin/users/add.php" method="POST">
        ...
        <div class="form-group">
            ...
            <select id="role" name="role">
                <option value="user" <?php echo (isset($_POST['role']) && $_POST['role'] == 'user') ? 'selected' : ''; ?>>Korisnik</option>
                <option value="admin" <?php echo (isset($_POST['role']) && $_POST['role'] == 'admin') ? 'selected' : ''; ?>>Administrator</option>
            </select>
        </div>
        <div class="form-group">
            <label for="is_active">
                <input type="checkbox" id="is_active" name="is_active" value="1" <?php echo (isset($_POST['is_active']) || !isset($_POST['username'])) ? 'checked' : ''; ?>> Aktivan
            </label>
        </div>
        <button type="submit">Dodaj korisnika</button>
        <a href="<?php echo BASE_URL; ?>/admin/users/index.php" class="button" style="margin-left: 10px;">Natrag na popis</a>
    </form>
</div>
