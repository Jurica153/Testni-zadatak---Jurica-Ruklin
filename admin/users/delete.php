...
if ($user_id > 0) {
    if ($user_id == $_SESSION['user_id']) {
        redirect('/admin/users/index.php?message_type=error&message=Ne možete obrisati vlastiti administratorski račun!');
    }

    $stmt = $pdo->prepare("DELETE FROM users WHERE id = :id");
    if ($stmt->execute(['id' => $user_id])) {
        redirect('/admin/users/index.php?message_type=success&message=Korisnik uspješno obrisan!');
    } else {
        redirect('/admin/users/index.php?message_type=error&message=Greška prilikom brisanja korisnika.');
    }
} else {
    redirect('/admin/users/index.php?message_type=error&message=Nevažeći ID korisnika za brisanje.');
}
?>
