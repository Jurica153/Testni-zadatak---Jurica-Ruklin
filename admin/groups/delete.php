...
if ($group_id > 0) {
    ...
    try {
        $stmt_delete_user_groups = $pdo->prepare("DELETE FROM user_groups WHERE group_id = :group_id");
        $stmt_delete_user_groups->execute(['group_id' => $group_id]);

        $stmt_delete_newsletter_targets = $pdo->prepare("DELETE FROM newsletter_targets WHERE group_id = :group_id");
        $stmt_delete_newsletter_targets->execute(['group_id' => $group_id]);

        $stmt = $pdo->prepare("DELETE FROM groups WHERE id = :id");
        $stmt->execute(['id' => $group_id]);

        $pdo->commit();
        redirect('/admin/groups/index.php?message=Grupa uspješno obrisana!');
    } catch (Exception $e) {
        $pdo->rollBack();
        redirect('/admin/groups/index.php?message=Greška prilikom brisanja grupe: ' . urlencode($e->getMessage()));
    }
} else {
    redirect('/admin/groups/index.php?message=Nevažeći ID grupe za brisanje.');
}
?>
