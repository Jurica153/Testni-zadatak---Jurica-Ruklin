    ...
    error_log("DEBUG: Current user_id in session: " . ($_SESSION['user_id'] ?? 'N/A'));
    echo "DEBUG: Current user_id in session: " . ($_SESSION['user_id'] ?? 'N/A') . "<br>";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($title) || empty($subject) || empty($content)) {
            ...
            try {
                $stmt = $pdo->prepare("INSERT INTO newsletters (title, subject, content, status, scheduled_at, created_by) VALUES (:title, :subject, :content, :status, :scheduled_at, :created_by)");
                $stmt->execute([
                    'title' => $title,
                    'subject' => $subject,
                    'content' => $content,
                ...
                $pdo->commit();
                redirect('/admin/newsletters/index.php?message=Newsletter uspješno kreiran!');
            } catch (Exception $e) {
                $pdo->rollBack();
                $error = "Greška prilikom kreiranja newslettera: " . $e->getMessage();
            }
        }
    }
    ...
    <div class="container">
        <h2>Kreiraj novi Newsletter</h2>
        <?php if (!empty($error)): ?>
            <p class="error-message"><?php echo htmlspecialchars($error); ?></p>
        <?php endif; ?>
        <form action="<?php echo BASE_URL; ?>/admin/newsletters/create.php" method="POST">
            ...
            <div class="form-group">
                ...
                <textarea id="newsletter_content" name="content"><?php echo isset($_POST['content']) ? htmlspecialchars($_POST['content']) : ''; ?></textarea>
            </div>
            <div class="form-group">
                <label for="status">Status:</label>
                <select id="status" name="status">
                    <option value="draft" <?php echo (isset($_POST['status']) && $_POST['status'] == 'draft') ? 'selected' : ''; ?>>Nacrt</option>
                    <option value="scheduled" <?php echo (isset($_POST['status']) && $_POST['status'] == 'scheduled') ? 'selected' : ''; ?>>Zakazano</option>
                </select>
            </div>
            <div class="form-group">
                <label for="scheduled_at">Zakazano za (datum i vrijeme, opcionalno):</label>
                <input type="datetime-local" id="scheduled_at" name="scheduled_at" value="<?php echo isset($_POST['scheduled_at']) ? htmlspecialchars($_POST['scheduled_at']) : ''; ?>">
            </div>
                    ...
                    <?php foreach ($all_groups as $group_option): ?>
                        <label>
                            <input type="checkbox" name="target_groups[]" value="<?php echo htmlspecialchars($group_option['id']); ?>"
                                <?php echo (isset($_POST['target_groups']) && in_array($group_option['id'], $_POST['target_groups'])) ? 'checked' : ''; ?>>
            ...
            <button type="submit">Kreiraj Newsletter</button>
